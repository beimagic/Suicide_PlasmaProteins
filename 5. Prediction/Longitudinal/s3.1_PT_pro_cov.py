...
output_file = dpath + 'Results/'+type+'_HoldOut/s3.1_ParameterTuning_'+imp_type+'_pro_cov.csv'
...
target_df = pd.read_csv(dpath + 'Data/suicide_'+type+'.csv', usecols = ['eid', 'target_y', 'Age', 'Sex', 'TrainTestSplit', 'FoldSplit'])
target_df = target_df.loc[target_df.TrainTestSplit == 0]
mydf = pd.merge(target_df, pro_df, how = 'inner', on = ['eid'])

my_params_lst = Parallel(n_jobs=nb_cpus)(delayed(params_iter)(mydf, ['Age', 'Sex']+pro_f_lst, fold_id_lst, my_seed, my_params) for my_params in candidate_params_lst)
