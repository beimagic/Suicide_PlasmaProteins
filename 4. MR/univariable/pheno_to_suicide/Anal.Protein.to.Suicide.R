...
for (f in c(1:nrow(ls.IM))){
      ...
      traitB='suicide'
      ##exposure dat
      exposure_dat=as.data.table(fread(paste0(path_exposure, "Expo_", traitA), header=T,colClasses=NULL))
      ## outcome dat
      outcome_dat = as.data.table(fread(file=paste0(path_outcome, traitA, "_to_suicide"),sep='\t'))
