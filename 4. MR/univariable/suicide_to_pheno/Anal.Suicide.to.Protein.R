...
for (f in c(1:nrow(ls.IM))){
      ...
      ## exposure dat
      exposure_dat=as.data.table(fread(paste0(path_exposure, "suicide.expo_dat_1e-6"), header=T,colClasses=NULL))
      ## outcome dat
      outcome_dat = as.data.table(fread(paste0(path_outcome, "outcome_", traitB),sep='\t'))
      
      # Harmonise the exposure and outcome data
      dat <- harmonise_data(exposure_dat, outcome_dat)
      
      # calculate MR,method_list =c("mr_ivw","mr_egger_regression","mr_weighted_median")
      MR=mr(dat = dat)
      if (f==1){
        MRsummary=MR
      }else{
      ...
      ggsave(fig.scatter[[1]], file=paste0(path_leave, traitA,'_',traitB,"_scatter_plot.png"), width=7, height=7)
      
      #forest plot
      res_single <- mr_singlesnp(dat)
      fig.forest = mr_forest_plot(res_single)
      ggsave(fig.forest[[1]], file=paste0(path_leave, traitA,'_',traitB,"_forestr_plot.png"), width=7, height=7)
      
      #loo plot
      res_loo = mr_leaveoneout(dat)
      fig.loo = mr_leaveoneout_plot(res_loo)
      ggsave(fig.loo[[1]], file=paste0(path_leave, traitA,'_',traitB,"_loo_plot.png"), width=7, height=7)
      
      #funnel plot
      fig.funnel=mr_funnel_plot(res_single)
      ggsave(fig.funnel[[1]], file=paste0(path_leave, traitA,'_',traitB,"_funnel_plot.png"), width=7, height=7) 
